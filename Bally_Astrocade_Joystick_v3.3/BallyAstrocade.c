/* Bally/Astrocade joystick to USB
...
#include <string.h>
#include "usbconfig.h"
#include "BallyAstrocade.h"

#define SETUPDELAY	7//5
#define DIVIDER 4 // 50K pot
#define TIMEOUT	65000

static char BallyAstrocadeInit(void);
static void BallyAstrocadeUpdate(void);
static char BallyAstrocadeChanged(char id);
static char BallyAstrocadeBuildReport(unsigned char *reportBuffer, char id);
static void BallyAstrocadeReadPot(void);
...
static char BallyAstrocadeInit(void)
{
	 ...
	 * PIN3-PB2 = (I,1) LEFT
	 * PIN4-PB3 = (I,1) RIGHT
	 * PIN5-PC1 = (I,0) nc, PC3 = (I,1) TRIGGER
	 * PIN6-PB4 = (I,1) UP
	 * PIN7-PB5 = (O,1) POT END (+5V)
	 * PIN8-PD7 = (O,0) GND
	 * PIN9-PC0 = (I,0) POT, PC2 = (I,0) nc
	 */
	...
	ADMUX=0; // Channel 0 selected (PC0)
...
static void BallyAstrocadeUpdate(void)
{
	...
	last_update_state = ((PINB&((1<<PB1)|(1<<PB2)|(1<<PB3)|(1<<PB4))) | ((PINC&(1<<PC3))>>3));
	BallyAstrocadeReadPot();
...
static char BallyAstrocadeBuildReport(unsigned char *reportBuffer, char id)
{
	...
	if (reportBuffer)
	{
		...
		reportBuffer[2] = z;
		reportBuffer[3] = 0;
		if (tmp&(1<<PB0)) reportBuffer[3] |= 0b00000001; // FIRE
...
const char BallyAstrocade_usbHidReportDescriptor[] PROGMEM = {
...
const char BallyAstrocade_usbDescrDevice[] PROGMEM = {    /* USB device descriptor */
...
Gamepad BallyAstrocadeJoy = {
	.num_reports			=	1,
	.reportDescriptorSize	=	sizeof(BallyAstrocade_usbHidReportDescriptor),
	.deviceDescriptorSize	=	sizeof(BallyAstrocade_usbDescrDevice),
	.init					=	BallyAstrocadeInit,
	.update					=	BallyAstrocadeUpdate,
	.changed				=	BallyAstrocadeChanged,
	.buildReport			=	BallyAstrocadeBuildReport,
};

Gamepad *BallyAstrocadeGetGamepad(void)
{
	BallyAstrocadeJoy.reportDescriptor = (void*)BallyAstrocade_usbHidReportDescriptor;
	BallyAstrocadeJoy.deviceDescriptor = (void*)BallyAstrocade_usbDescrDevice;

	return &BallyAstrocadeJoy;
}
...
void BallyAstrocadeReadPot(void)
{
	flag=1;	// Read in process flag
	DDRC |= (1<<PC0);	// Force port to ground
	_delay_us(SETUPDELAY);	// Wait for capacitor to be discharged
	DDRC &= ~(1<<PC0);	// Put back port in read mode (floating);
